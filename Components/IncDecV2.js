...
const IncDecV2 = ({ min = 0, max = 500000, defaultValue = 0, onCange = (type, value) => {} }) => {
  ...
  return (
    <div className="grid grid-flow-col gap-x-[1.2rem] ">
      <button onClick={onDec} className="h-[5.1rem] mobile:h-[5.1rem] w-[4.6rem] mobile:w-[4.6rem] grid place-items-center bg-[#F6EBF5] rounded-[1rem] border-2 border-primary cursor-pointer">
      ...
      <button onClick={onInc} className=" h-[5.1rem] mobile:h-[5.1rem] w-[4.6rem] mobile:w-[4.6rem] grid place-items-center bg-[#F6EBF5] rounded-[1rem] border-2 border-primary cursor-pointer">
        <span className="font font-medium text-[2.6rem] mobile:text-[2.6rem]">+</span>
      </button>
    </div>
  );
